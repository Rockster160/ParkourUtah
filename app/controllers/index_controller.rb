...
class IndexController < ApplicationController
  ...
  def receive_sms
    ...
    if %w(STOP STOPALL UNSUBSCRIBE CANCEL END QUIT).include?(params["Body"].squish.upcase)
      ...
      user_message = user.present? ? "\nPhone Number seems to match: <#{admin_user_url(user, host: 'http://parkourutah.com, port: nil')}|#{user.id} - #{user.email}>" : ""
      ...
      respond_message = "\n<#{batch_text_message_admin_url(recipients: raw_number, host: 'http://parkourutah.com, port: nil')}|Click here to respond!>"
