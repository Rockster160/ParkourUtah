  ...
  def displayable_attributes
    {
      ...
      key: self.key,
      request_uri: "#{self.request_uri.split(" ")[1]}",
      http_status: "#{self.http_status.presence || 'Unknown'} (#{Rack::Utils::HTTP_STATUS_CODES[self.http_status.to_i].presence || 'Unknown'})",
      error_code: self.error_code,
      bytes_sent: "#{number_to_human_size(self.bytes_sent, precision: 3)} \n(#{number_with_delimiter(self.bytes_sent)} bytes)",
      object_size: "#{number_to_human_size(self.object_size, precision: 3)} \n(#{number_with_delimiter(self.object_size)} bytes)",
